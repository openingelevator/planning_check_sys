...
class MaskRCNNAgent(BaseAgent):
    ...
    def get_maskrcnn_prediction(self, frame, confidence):
        ...
        # Extract scores, masks, labels, and boxes
        pred_score = pred[0]['scores'].detach().cpu().numpy()
        pred_pruned = pred_score > confidence  # Boolean array of shape (N,)

        if pred_pruned.any():  # If there are any predictions above the confidence threshold
            masks = (pred[0]['masks'] > 0.5).detach().cpu().numpy()  # Shape (N, 1, H, W)
            masks = masks[:, 0, :, :]  # Remove the second dimension, shape (N, H, W)

            pred_boxes = pred[0]['boxes'].detach().cpu().numpy()  # Shape (N, 4)
            pred_class = [self.classes[i] for i in pred[0]['labels'].detach().cpu().numpy()]  # List of strings

            # Slice masks, boxes, and classes based on confidence threshold
            masks = masks[pred_pruned]
            pred_boxes = [[(int(box[0]), int(box[1])), (int(box[2]), int(box[3]))] for box in pred_boxes[pred_pruned]]
            pred_class = [pred_class[i] for i in np.where(pred_pruned)[0]]
        ...
        if self.debug:
            ...
            for i in range(len(masks)):
                ...
                if rgb_mask.shape == img.shape:
                    ...
                    pt1 = (int(boxes[i][0][0]), int(boxes[i][0][1]))  # 转换为整数
                    pt2 = (int(boxes[i][1][0]), int(boxes[i][1][1]))  # 转换为整数
                    cv2.rectangle(img, pt1, pt2, color=(0, 255, 0), thickness=2)
                    cv2.putText(img, pred_cls[i], pt1, cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),thickness=2)
